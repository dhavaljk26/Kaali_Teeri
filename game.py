...
@app_game.route('/toggle_clear_timer', methods=['POST'])
@login_required
def toggle_clear_timer():
    global is_clear_timer
    if is_clear_timer:
        is_clear_timer = False
    else:
        is_clear_timer = True
...
def create_deck(number_of_decks, number_of_players):
    ...
    points = [10, 0, 0, 0, 5, 0, 0, 0, 0, 10, 10, 10, 10] if not playing_mindi else [1, 1, 1, 1, 1, 1, 1, 1, 1, 100, 1,
                                                                                     1, 1]
...
@login_required
def bid():
    global players, bidders, bidding_completed, game, player_order, rounds, player_points, is_clear_timer
    if playing_mindi:
...
def bid_post():
    ...
    if bidding_completed == False:
        ...
        if current_user.name in bidders == True:
            return render_template('bid.html', already_bid=(current_user.name in bidders), is_clear_timer = is_clear_timer,
                                   cards=sorted(hand.cards, key=lambda x: (x.suit, x.value)))
        ...
        return render_template('bid.html', already_bid=True, is_clear_timer = is_clear_timer, cards=sorted(hand.cards, key=lambda x: (x.suit, x.value)),
                               activityClass="inactiveLink")
...
def choose_trump_and_partner():
    global game, players,bidding_completed,game_started
    ...
    if game_started == False:
        return list_players()
    elif bidding_completed == False:
        return bid()
    elif game.bidder == current_user.name:
        to_choose = True
    hand = get_hand()
    return render_template('/choose.html', to_choose=to_choose, suits=suits, values=values, turn=turn,
...
def post_choose_trump_and_partner():
    ...
    for i in range(1, number_of_partners + 1):
        game.partners.append(
            Partner(suit=request.form.get('partner_suit' + str(i)), value=request.form.get('partner_value' + str(i)),
                    turn_number=int(request.form.get('partner_turn' + str(i)))))
...
def play_round(round_id):
    ...
    if len(player_order) <= player_shift:
        ...
        for i, card in enumerate(table_cards):
            if card.suit == start_suit and rank[card.value] >= rank[table_cards[winner].value]:
                winner = i
            elif playing_mindi and card.suit != start_suit and not game.trump:
    ...
    return render_template('round.html', round_id=round_id, cards=sorted(hand.cards, key=lambda x: (x.suit, x.value)),
                           trump=game.trump, partner_cards=partner_cards, table_cards=table_cards,
                           activityClass=activityClass, turn_id=player_shift, past_rounds=past_rounds,
                           player_order=player_order, bid_winner=bid_winner, bid=game.bid, player_points=player_points,
                           suit_exists=suit_exists)
