...
@login_required
def bid():
    global players, bidders, bidding_completed, game, player_order, rounds, player_points
    if playing_mindi:
    ...
    hand = get_hand()
    return render_template('bid.html', already_bid=already_bid,
...
def bid_post():
    ...
    if bidding_completed == False:
        ...
        if current_user.name in bidders == True:
            return render_template('bid.html', already_bid=(current_user.name in bidders),
        ...
        return render_template('bid.html', already_bid=True, cards=sorted(hand.cards, key=lambda x: (x.suit, x.value)),
